  ...
  try {
    ...
    if (action === 'overview') {
      const { data: sessions } = await supabase
        ...
        .select('visitor_id, session_id, is_bounce, duration_seconds, is_repeat_visitor, started_at, ended_at')
    ...
    if (action === 'visitors-over-time') {
      const { data } = await supabase.from('analytics_sessions').select('started_at').gte('started_at', startDate).lte('started_at', endDate).order('started_at');
      const grouped = (data || []).reduce((acc: Record<string, number>, session) => { const date = session.started_at.split('T')[0]; acc[date] = (acc[date] || 0) + 1; return acc; }, {});
    ...
    if (action === 'traffic-sources') {
      const { data } = await supabase.from('analytics_sessions').select('utm_source, referrer').gte('started_at', startDate).lte('started_at', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { const source = item.utm_source || (item.referrer ? new URL(item.referrer).hostname : 'direct'); acc[source] = (acc[source] || 0) + 1; return acc; }, {});
    ...
    if (action === 'top-pages') {
      const { data } = await supabase.from('analytics_pageviews').select('path').gte('viewed_at', startDate).lte('viewed_at', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { acc[item.path] = (acc[item.path] || 0) + 1; return acc; }, {});
    ...
    if (action === 'device-breakdown') {
      const { data } = await supabase.from('analytics_sessions').select('device_type').gte('started_at', startDate).lte('started_at', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { const device = item.device_type || 'unknown'; acc[device] = (acc[device] || 0) + 1; return acc; }, {});
    ...
    if (action === 'airbnb-clicks-over-time') {
      const { data } = await supabase.from('analytics_outbound_clicks').select('timestamp').ilike('destination_url', '%airbnb%').gte('timestamp', startDate).lte('timestamp', endDate).order('timestamp');
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { const date = item.timestamp.split('T')[0]; acc[date] = (acc[date] || 0) + 1; return acc; }, {});
    ...
    if (action === 'airbnb-clicks-by-source') {
      const { data } = await supabase.from('analytics_outbound_clicks').select('utm_source').ilike('destination_url', '%airbnb%').gte('timestamp', startDate).lte('timestamp', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { const source = item.utm_source || 'direct'; acc[source] = (acc[source] || 0) + 1; return acc; }, {});
    ...
    if (action === 'airbnb-clicks-by-page') {
      const { data } = await supabase.from('analytics_outbound_clicks').select('page_path').ilike('destination_url', '%airbnb%').gte('timestamp', startDate).lte('timestamp', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { acc[item.page_path] = (acc[item.page_path] || 0) + 1; return acc; }, {});
    ...
    if (action === 'airbnb-clicks-by-device') {
      const { data } = await supabase.from('analytics_outbound_clicks').select('device_type').ilike('destination_url', '%airbnb%').gte('timestamp', startDate).lte('timestamp', endDate);
      const grouped = (data || []).reduce((acc: Record<string, number>, item) => { const device = item.device_type || 'unknown'; acc[device] = (acc[device] || 0) + 1; return acc; }, {});
