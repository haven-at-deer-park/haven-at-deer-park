...
import { Play, Volume2, VolumeX, Instagram, MessageCircle } from 'lucide-react';
  ...
  return (
    <section className="section bg-card/60 relative overflow-hidden">
      ...
      <div className="container relative z-10">
        ...
        {/* Social Banners */}
        <div className="mt-16 sm:mt-24 grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto animate-fade-in [animation-delay:600ms]">
          <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" 
             className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 p-[2px] transition-transform hover:scale-[1.02] shadow-lg">
            <div className="flex items-center justify-between gap-4 rounded-2xl bg-white/95 px-6 py-5 dark:bg-zinc-900/95 backdrop-blur-xl h-full">
              <div>
                <h4 className="font-bold text-lg dark:text-white">Follow us on Instagram</h4>
                <p className="text-sm text-muted-foreground mt-1">See more behind-the-scenes and guest stories.</p>
              </div>
              <div className="rounded-full bg-pink-100 p-3 text-pink-600 dark:bg-pink-900/40 dark:text-pink-400 group-hover:scale-110 transition-transform">
          ...
          <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" 
             className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-black to-zinc-600 dark:from-white dark:to-zinc-300 p-[2px] transition-transform hover:scale-[1.02] shadow-lg">
            <div className="flex items-center justify-between gap-4 rounded-2xl bg-white/95 px-6 py-5 dark:bg-zinc-900/95 backdrop-blur-xl h-full">
              <div>
                <h4 className="font-bold text-lg dark:text-white">Watch on TikTok</h4>
                <p className="text-sm text-muted-foreground mt-1">Join the community and explore our viral tours.</p>
              </div>
              <div className="rounded-full bg-zinc-100 p-3 text-zinc-900 dark:bg-zinc-800 dark:text-white group-hover:scale-110 transition-transform">
                {/* Fallback to MessageCircle since lucide-react doesn't have native TikTok out of box without custom SVG */}
